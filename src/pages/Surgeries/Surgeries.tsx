...
export default function Surgeries() {
  ...
  useEffect(() => {
    dispatch(getSurgeries(1))
      .unwrap()
      .then((result) => {
        setTotalRows(result.pagination.total);
        setTotalSurgeries(result.data);
      })
      .catch((error) => {
  ...
  const handleDelete = () => {
    dispatch(deleteSurgery(id.current.toString()))
      .unwrap()
      .then(() => {
        setTotalSurgeries((prev) =>
          prev.filter((el) => {
      ...
      renderCell: (params) => {
        return (
          <div className="flex items-center gap-5 mt-3">
            <Link to={`/surgeries/edit/${params.id}`}>
              <svg
  ...
  const handleChangePage = (e: { page: number }) => {
    ...
    dispatch(getSurgeries(e.page + 1))
      .unwrap()
      .then((result) => {
        setTotalRows(result.pagination.total);
        setTotalSurgeries(result.data);
