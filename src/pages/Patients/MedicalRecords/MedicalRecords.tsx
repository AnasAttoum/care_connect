...
export default function MedicalRecords() {
  ...
  const { loading, loadingDelete } = useSelector((state: RootState) => state.medicalRecord)
  const [totalMedicalRecords, setTotalMedicalRecords] = useState<medicalRecord[]>([])
  ...
  const columns: GridColDef[] = [
    ...
    {
      field: 'blood_type', headerName: 'Blood', width: 60
    },
    {
      field: 'admission_date', headerName: 'Admission Date', width: 120,
      renderCell: (params) => {
        return (
          <span>{params.row.admission_date}</span>
        );
      }
    },
    {
      field: 'discharge_date', headerName: 'Discharge Date', width: 120,
      renderCell: (params) => {
        return (
          <span>{params.row.discharge_date}</span>
        );
      }
    },
    {
    ...
    <>
      {
        loading === 'pending' ?
          ...
          <>
            ...
            <FloatingButton url={`/patients/${id}/add`} tooltip='Add Medical Record' />
          </>
      }
    </>
  )
