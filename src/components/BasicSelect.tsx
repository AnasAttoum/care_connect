...
export default function BasicSelect({ val, setVal, error, name, label, data }: {
  val: string | number, setVal: React.Dispatch<React.SetStateAction<{
    room_number: number, status: string, department_id: number, type: string, beds_number: number
  }>>, error: string, name: string, label: string, data: { id: number | string; name: string; }[]
}) {

  const handleChange = (event: SelectChangeEvent) => {
    setVal(prev => ({ ...prev, [name]: event.target.value as string }));
  };
